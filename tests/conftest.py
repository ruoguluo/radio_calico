    ...
    with app.app_context():
        ...
        def create_user():
            ...
            try:
                ...
                return (
                    jsonify({"id": user.id, "message": "User created successfully"}),
                    201,
                )
        ...
        def get_ratings(song_id):
            ...
            try:
                ...
                thumbs_up = SongRating.query.filter_by(
                    song_id=song_id, rating=1
                ).count()
                thumbs_down = SongRating.query.filter_by(
                    song_id=song_id, rating=-1
                ).count()

                user_fingerprint = generate_user_fingerprint(request)
                user_rating = SongRating.query.filter_by(
                    song_id=song_id, user_fingerprint=user_fingerprint
                ).first()
