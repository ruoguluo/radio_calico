...
import os
import tempfile
from app import app, init_db, get_db_connection, generate_user_fingerprint
...
class RadioCalicoTestCase(unittest.TestCase):
    ...
    def test_rate_song_thumbs_up(self):
        ...
        response = self.client.post(f'/api/ratings/{song_id}',
    ...
    def test_rate_song_invalid_rating(self):
        ...
        data = json.loads(response.data)
        self.assertIn('error', data)
        self.assertEqual(data['error'], 'Rating must be 1 (thumbs up) or -1 (thumbs down)')
    ...
    def test_update_existing_rating(self):
        ...
        rating_data = {'rating': 1}
        response1 = self.client.post(f'/api/ratings/{song_id}',
                                   data=json.dumps(rating_data),
                                   content_type='application/json')
        ...
        rating_data = {'rating': -1}
        response2 = self.client.post(f'/api/ratings/{song_id}',
                                   data=json.dumps(rating_data),
                                   content_type='application/json')
    ...
    def test_get_ratings_after_voting(self):
        ...
        # Rate the song
        rating_data = {'rating': 1}
        self.client.post(f'/api/ratings/{song_id}',
                        data=json.dumps(rating_data),
                        content_type='application/json')
